...
export default {
      ...
      if (window.gapi && Object.keys(window.gapi).length) {
        gapi.load('auth2', function () {
          that.googleAuth2 = gapi.auth2.init({
            client_id: env.ggParam.clientId,
            cookiepolicy: env.ggParam.cookiePolicy,
            scope: env.ggParam.scope
          })
        })
      } else {
        setTimeout(function () {
  ...
  created () {
    this.params = {
      sdkVer: env.loginParam.sdkVer,
      sdkType: env.loginParam.sdkType,
      runPlatform: env.loginParam.runPlatform
    }
  },
  beforeMount () {
  },
