...
export function viewPage (pageId, pageName) {
  ...
  const browser = BrowserUtils.getBrowserVersion()[0] || '_unknown'
  const browserVersion = BrowserUtils.getBrowserVersion()[1] || '_unknown'
...
function fetch (obj) {
  services.analyticsApi(env.API_PATH + '/api/analytics/source/tracking', obj, 'POST').then(
    function (response) {
